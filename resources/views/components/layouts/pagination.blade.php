    ...
    <div class="flex items-center space-x-3">
      ...
      @if ($paginator->onFirstPage())
        <x-buttons.button
          ...
          <svg class="w-5 h-5 mr-1 -ml-1" fill="gray">
            <use xlink:href="{{ asset("sprite.svg#previous") }}"></use>
          </svg>
          Previous
        </x-buttons.button>
      @else
      ...
      @if ($paginator->hasMorePages())
        <x-buttons.button
          ...
          Next
          <svg class="w-5 h-5 ml-1 -mr-1" fill="gray">
            <use xlink:href="{{ asset("sprite.svg#next") }}"></use>
          </svg>
